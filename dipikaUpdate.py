    ...
    for word in inventoryList:
        ...
        while (a<=5):
            if(counter%50 == 0):
                ...
                inObj1 = Inventory(minimum_qty = int(word[6]), qty = int(word[5]),batch_id = 00001,cost_price = Decimal(word[4]),selling_price = Decimal(1.05)*Decimal(word[4]) ,product_id_id = int(word[3]),store_id_id=a,expiry_date=datetime.date(2013,12,15),min_restock = 0.1*int(word[5]))
                inObj2 = Inventory(minimum_qty = int(word[6]), qty = int(word[5]),batch_id = 00002,cost_price = Decimal(1.1)*Decimal(word[4]),selling_price = Decimal(1.15)*Decimal(word[4]) ,product_id_id = int(word[3]),store_id_id=a,expiry_date=datetime.date(2013,12,16),min_restock = 0.1*int(word[5]))
            else:
                inObj1 = Inventory(minimum_qty = int(word[6]), qty = int(word[5]),batch_id = 00001,cost_price = Decimal(word[4]),selling_price = Decimal(1.05)*Decimal(word[4]) ,product_id_id = int(word[3]),store_id_id=a,min_restock = 0.1*int(word[5]))
                inObj2 = Inventory(minimum_qty = int(word[6]), qty = int(word[5]),batch_id = 00002,cost_price = Decimal(1.1)*Decimal(word[4]),selling_price = Decimal(1.15)*Decimal(word[4]) ,product_id_id = int(word[3]),store_id_id=a,min_restock = 0.1*int(word[5]))
            inObj1.save()
            inObj2.save()
            a = a + 1
        counter = counter + 1
